...
import { generateIncomingMessage, generateReply, evaluatePerformance } from '../../services/slackSimulationService';
...
export default function SlackSimulation() {
  ...
  const [typingUsers, setTypingUsers] = useState({}); // channelId -> userIds
  ...
  const handleSendMessage = async (text) => {
    ...
    if (activeScenario && activeScenario.channelId === selectedChannelId) {
      // Pick a stakeholder who isn't the current user
  ...
  const addMessage = (channelId, senderId, text) => {
    const newMessage = {
  ...
  const triggerTyping = (channelId, userId) => {
    setTypingUsers(prev => ({
      ...prev,
  ...
  useEffect(() => {
    const interval = setInterval(async () => {
      ...
      // If a scenario is active, reduce background noise
      const chance = activeScenario ? 0.2 : 0.7;
