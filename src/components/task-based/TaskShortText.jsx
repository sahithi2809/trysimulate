...
const TaskShortText = ({ 
  ...
  const handleSubmit = () => {
    const minWords = task?.config?.minWords || 5;
  ...
  const wordCount = response.trim().split(/\s+/).filter(Boolean).length;
  const minWords = task?.config?.minWords || 5;
  const maxWords = task?.config?.maxWords || null;
  const instruction = task?.config?.instruction || '';
  const prompt = task?.config?.prompt || '';
  const referenceData = task?.config?.referenceData || null;

  return (
    <div className="max-w-4xl mx-auto">
      <div className="bg-white rounded-xl shadow-xl p-8 mb-6">
        <h2 className="text-3xl font-bold text-gray-900 mb-4">{task?.name}</h2>
              ...
              {Object.entries(referenceData).map(([key, value]) => (
                <li key={key} className="flex items-center gap-2">
                  ...
                  <span className={`px-3 py-1 rounded-full text-sm font-semibold ${
                    ...
                    value === 'Low' ? 'bg-red-100 text-red-700' :
        ...
        <div className="mb-6">
          <label className="block text-sm font-bold text-gray-900 mb-2">
            Your Response
            {minWords && ` (Minimum ${minWords} words${maxWords ? `, Maximum ${maxWords} words` : ''})`}
          </label>
          <textarea
            value={response}
          ...
          <div className="mt-2 flex justify-between items-center">
            <p className={`text-sm ${wordCount < minWords ? 'text-red-600' : 'text-gray-600'}`}>
        ...
        {validationResult && (
          <div className="mb-6 p-4 rounded-xl border-2 bg-green-50 border-green-200">
            ...
            {validationResult.feedback && (
              <p className="text-sm text-gray-700 mt-2">{validationResult.feedback}</p>
            )}
          </div>
        )}
        ...
        <div className="flex justify-between items-center pt-6 border-t-2 border-gray-200">
          ...
          <div className="flex gap-3">
            <button
              ...
              disabled={submitted || wordCount < minWords || (maxWords && wordCount > maxWords)}
